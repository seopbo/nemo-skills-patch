...
def setup_workspace_and_mounts(workspace, cluster):
    ...
    for env_var in env_vars:
        if isinstance(env_var, str) and "HF_HOME=" in env_var:
            ...
            break
    if not hf_home:
        raise ValueError("HF_HOME is not set in the cluster config")
...
def start_server_only(workspace, cluster, expname_prefix, server_timeout, nim_config):
    """Mode 1: Only start the ASR NIM server."""
    ...
    # Use start_server to launch the ASR NIM server
    start_server(
        cluster=cluster,
        model="asr_nim",  # Dummy model name for ASR NIM
        server_type="generic",
        server_gpus=1,
        server_nodes=1,
...
def run_generation_only(workspace, cluster, expname_prefix, server_host, server_port, server_node, nim_config):
    ...
    # Test data is packaged with the code and ready to use
    input_file = "/nemo_run/code/tests/slurm-tests/asr_nim/asr.test"
    output_dir = f"{workspace}/asr_outputs"
...
def run_full_pipeline(workspace, cluster, expname_prefix, nim_config):
    ...
    generate(
        ctx=wrap_arguments("++generation_type=asr"),
        cluster=cluster,
...
def main():
    parser = argparse.ArgumentParser(description="Run ASR NIM slurm test in different modes")
    ...
    parser.add_argument(
        "--config_key",
        default="parakeet-tdt-0.6b-v2:1.0.0",
    ...
    if not args.skip_check:
        ...
        checker_script = "/nemo_run/code/tests/slurm-tests/asr_nim/check_results.py"
        checker_cmd = (
