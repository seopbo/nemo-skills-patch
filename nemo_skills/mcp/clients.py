...
# HTTP Connection Pool classes
class MCPStreamableHttpConnectionPool(MCPClient):
    """Semaphore-based concurrency limiter for MCP HTTP clients.

    Limits the number of concurrent HTTP MCP requests to avoid overwhelming the server.
    Each request still creates a fresh connection (via streamablehttp_client), but we
    limit how many can be in-flight at once using an asyncio.Semaphore.

    This approach avoids the anyio cancel scope issues that arise when trying to keep
    persistent connections with context managers alive across different async tasks.

    Args:
        base_url: HTTP endpoint URL (e.g., "http://127.0.0.1:8765/mcp")
        pool_size: Maximum number of concurrent requests (default: 4)
        lazy_init: Compatibility parameter, ignored (default: True)
        acquire_timeout: Optional timeout for acquiring semaphore (default: None)

    Usage:
        pool = MCPStreamableHttpConnectionPool(
            base_url="http://127.0.0.1:8765/mcp",
            pool_size=32,
            acquire_timeout=30.0,
        )

        # Concurrent calls automatically limited to pool_size
        results = await asyncio.gather(
            pool.call_tool("execute", {"code": "1+1"}),
            pool.call_tool("execute", {"code": "2+2"}),
        )
    ...
    async def _ensure_initialized(self) -> None:
        ...
        async with self._get_init_lock():
            ...
            self._initialized = True
            LOG.info(f"MCPStreamableHttpConnectionPool initialized with concurrency limit of {self.pool_size}")

    async def warm_up(self) -> None:
        """Pre-initialize the pool (creates semaphore).

        Note: Unlike persistent connection pools, this doesn't pre-connect anything.
        It's provided for API compatibility with connection pool expectations.
        """
        await self._ensure_initialized()
        LOG.info(f"MCPStreamableHttpConnectionPool ready (concurrency limit: {self.pool_size})")
    ...
    async def _acquire(self):
        ...
        try:
            ...
            raise TimeoutError(
                f"Could not acquire slot within {self.acquire_timeout}s. "
                f"Concurrency limit: {self.pool_size}, waiting requests: {self._waiting_count}"
            )
        finally:
            self._waiting_count -= 1
    ...
    async def list_tools(self):
        """List available tools (creates fresh connection)."""
        if self.tools:
            return self.tools
    ...
    async def call_tool(self, tool: str, args: dict) -> Any:
        """Execute a tool call with concurrency limiting (creates fresh connection)."""
        self._assert_tool_allowed(tool)
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "pool_size": self.pool_size,
            "available": available,
            "in_use": self._active_count,
            "connected": self._active_count,  # For compatibility
            "waiting_requests": self._waiting_count,
            "closed": self._closed,
        }

    async def close(self) -> None:
        """Close the pool (marks as closed)."""
        self._closed = True
        LOG.info(f"MCPStreamableHttpConnectionPool closed (concurrency limit was {self.pool_size})")
