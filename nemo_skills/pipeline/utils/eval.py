...
def get_benchmark_args_from_module(
    ...
    if benchmark in ["swe-bench", "terminal-bench"] and cluster_config["executor"] == "local":
        ...
        # terminal-bench runs docker inside docker to build images on-the-fly,
        # which requires mounting the docker binary & socket from the host machine
        if benchmark == "terminal-bench":
            os.environ["NEMO_SKILLS_MOUNT_HOST_DOCKER"] = "1"
