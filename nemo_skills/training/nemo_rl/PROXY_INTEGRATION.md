# NeMo-Skills Proxy Integration with NeMo-RL/NeMo-Gym

This document describes the design of the NeMo-Skills proxy server that enables running NeMo-Skills generation logic (prompt formatting, code execution, tool calling) within the NeMo-RL training loop via NeMo-Gym.

---

## 🚧 TODO: Production Readiness

This is a proof-of-concept. The following items must be addressed before adding to core pipelines, **sorted by design impact** (most impactful first):
...
eval_type = request_body.get("_eval_type", "math")
if eval_type and supports_single_eval(eval_type, {}):
    evaluator = get_evaluator_class(eval_type, {})
    eval_result = await evaluator.eval_single({
        "generation": generation_text,
        "expected_answer": request_body.get("expected_answer"),
        **output
    })
    reward = 1.0 if eval_result.get("is_correct") else 0.0
```
...
**Benefits:**
- Reuse existing battle-tested evaluation logic
...
│                              NeMo-RL Training                                │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────────────────────┐  │
│  │   GRPO       │───▶│  NemoGym     │───▶│  NeMo-Skills Proxy           │  │
│  │  Algorithm   │    │  Environment │    │  (generate.py + skills_proxy)│  │
│  └──────────────┘    └──────────────┘    └───────────────┬──────────────┘  │
│         │                    │                           │                   │
│         │                    │                           ▼                   │
│         │                    │           ┌──────────────────────────────┐   │
...
This allows the script to work without NeMo-Gym installed, falling back to `MathEnvironment`.
...
Total rollout batch must be divisible by training batch size:
...
policy:
  train_global_batch_size: 2  # Must divide total (2)
```
...
| Variable | Purpose |
|----------|---------|
| `NEMO_RL_VLLM_URL` | vLLM server URL (auto-discovered if not set) |
| `NEMO_RL_MODEL_NAME` | Model name for tokenize calls |
| `NEMO_GYM_HEAD_SERVER_URL` | NemoGym head server for config discovery |
