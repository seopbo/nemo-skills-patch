        ...
        if isinstance(prompt, list):
            ...
            if needs_chunking:
                ...
                for chunk_idx, audio_chunk in enumerate(chunks):
                    ...
                    # Post-process Qwen2-Audio output using AudioBench's EXACT implementation
                    # https://github.com/AudioLLMs/AudioBench/blob/main/src/model_src/qwen2_audio_7b_instruct.py
                    # Lines 42-62

                    # Additional step: Remove SRT subtitle timestamps (vLLM-specific issue)
                    ...
                    # === AudioBench's EXACT implementation ===

                    # Step 1: Extract from double quotes (non-greedy)
                    match = re.search(r'"((?:\\.|[^"\\])*)"', generation)
                    if match:
                        generation = match.group(1)

                    # Step 2: Handle colon-quote patterns
                    if ":'" in generation:
                        generation = "'" + generation.split(":'")[1]
                    elif ": '" in generation:
                        generation = "'" + generation.split(": '")[1]

                    # Step 3: ALWAYS extract from single quotes (greedy - matches LONGEST)
                    match = re.search(r"'(.*)'", generation)
                    if match:
                        generation = match.group(1)
