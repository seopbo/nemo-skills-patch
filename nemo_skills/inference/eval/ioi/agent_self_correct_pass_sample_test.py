...
class IOIExecutionGenerationTask(GenerationTask):
    ...
    async def process_single_datapoint(self, data_point, all_data, prompt=None):
        ...
        for step_num in range(num_steps_completed, self.cfg.total_steps):
            ...
            # Update memory with current code solution and score using probabilistic replacement
            if int(self.cfg.show_k_solutions) > 0:
                current_code_block = extract_code_block(cur_generation_response)
                self._update_memory_sampling(
    ...
    def _update_memory_sampling(self, code: str, step_index: int, sample_score: float, chat_history_ref: list) -> None:
        """Keep only solutions that pass all sample tests. When memory is full,
        replace a random stored solution with probability p."""
        ...
        # Only keep solutions that pass ALL sample tests
        if float(sample_score) < 1.0:
            if chat_history_ref:
                chat_history_ref[-1]["memory_candidates"] = [e.get("solution") for e in self.saved_solutions]
            return
        ...
        # Memory full: replace with probability p
        if random.random() < float(self.cfg.memory_replace_prob):
            idx = random.randrange(k)
            self.saved_solutions[idx] = {
                "solution": code,
                "step": int(step_index),
                "sample_score": float(sample_score),
            }
    ...
    def _build_previous_solutions_text(self, k: int) -> str:
        ...
        # Take up to k most recent from memory, in reverse-chronological order
        tail = self.saved_solutions[-k:]
